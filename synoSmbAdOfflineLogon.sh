...
if [ $(id -u "$(whoami)") -ne 0 ]; then
	echo "synoSmbAdOfflineLogon needs to run as root!"
	exit 1
fi
...
if [ ! -z "${git}" ] && [ -d "$(dirname "$0")/.git" ] && [ -f "$(dirname "$0")/autoupdate" ]; then
	if [ ! -f /tmp/.synoSmbAdOfflineLogonUpdate ] || [ "${today}" != "$(date -r /tmp/.synoSmbAdOfflineLogonUpdate +'%Y-%m-%d')" ]; then
		echo "Checking for updates..."
		# touch file to indicate update has run once
		touch /tmp/.synoSmbAdOfflineLogonUpdate
		...
		if [ $commits -gt 0 ]; then
			...
			exec "$(pwd -P)/synoSmbAdOfflineLogon.sh" "$@"
...
if ! grep -q 'winbind offline logon=yes' "/etc/samba/smb.conf"; then
